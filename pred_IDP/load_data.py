...
# @Last Modified time: 2022-12-27 11:22:49
"""
# 数据集、特征 原始文件加载
...
def file_2_data(data_file_name):
	...
	for i in range(len(data_list)):
		...
		one_BERT_path = os.path.split(data_file_name)[0] + '/' + os.path.split(data_file_name)[1].split('.')[0].split('_')[-1] + '/DisoBERT/' + one_seq_id+ '.npy'
		# print(one_BERT_path)
...
def file_2_data_4_CAID(data_file_name):
	...
	for i in range(len(data_list)):
		...
		one_BERT_path = '.././dataset/CAID_data/test/DisoBERT/' + one_seq_id+ '.npy'
		one_BERT_vec = np.load(one_BERT_path,allow_pickle=True)
		one_BERT_vec = one_BERT_vec.reshape(len(one_BERT_vec),-1)
		seq_feature.append(one_BERT_vec)   # 序列BERT特征
...
def file_2_data_test(data_file_name):
	...
	for i in range(len(data_list)):
		...
		one_BERT_path =  'temp/embeddings/IDP_BERT/' + one_seq_id+ '.npy'
		# print(one_BERT_path)
		one_BERT_vec = np.load(one_BERT_path,allow_pickle=True)
		one_BERT_vec = one_BERT_vec.reshape(len(one_BERT_vec),-1)
		seq_feature.append(one_BERT_vec)   # 序列BERT特征
