...
# @Last Modified time: 2022-12-27 11:31:44
"""
# 数据集、特征 原始文件加载
...
def file_2_data(data_file_name):
	...
	for i in range(len(data_list)):
		...
		one_BERT_path = os.path.split(data_file_name)[0] + '/' + os.path.split(data_file_name)[1].split('.')[0].split('_')[-1] + '/T5/' + one_seq_id+ '.npy'
		# print(one_BERT_path)
...
def file_2_data_4_CAID(data_file_name):
	...
	for i in range(len(data_list)):
		...
		one_BERT_path = 'CAID_data/T5/' + one_seq_id+ '.npy'
		one_BERT_vec = np.load(one_BERT_path,allow_pickle=True)
		one_BERT_vec = one_BERT_vec.reshape(len(one_BERT_vec),-1)
		seq_feature.append(one_BERT_vec)   # 序列BERT特征
...
# 	test_dir = "fIDPnn_data/fIDPnn_train.txt"
# 	seq_id,seq,seq_label,seq_feature = file_2_data(test_dir)
	
# 	for i in range(len(seq_id)):
# 		if len(seq[i]) != len(seq_label[i]) or len(seq[i]) != np.array(seq_feature[i]).shape[0]:
# 			print(np.array(seq_id[i]))
# 			print(len(seq[i]))
# 			print(len(seq_label[i]))

# 			print(np.array(seq_feature[i]).shape)
# 		print("no")
...
def file_2_data_test(data_file_name):
	...
	for i in range(len(data_list)):
		...
		one_BERT_path = 'temp/embeddings/T5/' + one_seq_id+ '.npy'
		# print(one_BERT_path)
		one_BERT_vec = np.load(one_BERT_path,allow_pickle=True)
		one_BERT_vec = one_BERT_vec.reshape(len(one_BERT_vec),-1)
		seq_feature.append(one_BERT_vec)   # 序列BERT特征
